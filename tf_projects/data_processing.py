...
def split_data(data, val_size=0.1, test_size=0.1):
    ...
    df_train, df_val, df_test = data.iloc[:nval], data.iloc[
        nval:ntest], data.iloc[ntest:]
...
def load_csvdata(rawdata, time_steps, seperate=False):
    ...
    train_x, val_x, test_x = prepare_data(data['a']
                                          if seperate else data, time_steps)
    train_y, val_y, test_y = prepare_data(
        data['b'] if seperate else data, time_steps, labels=True)
    return dict(
        train=train_x, val=val_x, test=test_x), dict(
